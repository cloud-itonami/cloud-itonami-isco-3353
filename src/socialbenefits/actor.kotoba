(ns socialbenefits.actor
  "SocialBenefitsActor — the ISCO-08 3353 government social benefits
  officials documentation/logistics-coordination actor as a
  `langgraph.graph/state-graph` (ADR-2607121000 / CLAUDE.md Actors
  section). One graph run = one documentation/logistics operation
  ...
  resume after human sign-off. Modeled on cloud-itonami-isco-3333's
  placementops.actor.
  ...
  The unconditional invariant: the Case Documentation Advisor can never
  directly commit a record the SocialBenefitsGovernor refuses — every
  commit-record! call is gated behind `:decide` — and NO commit path
  can ever approve, deny or terminate a benefit, or otherwise exercise
  a benefits-eligibility-determination authority (permanently out of
  scope, structurally absent from the op allowlist, see
  `socialbenefits.governor`). Every observation suggesting an
  eligibility decision is needed can ONLY reach a human caseworker via
  the always-escalating `:flag-eligibility-review` op."
...
(defn build-graph
  "Build a compiled SocialBenefitsActor graph. `store` implements
  `socialbenefits.store/Store`. `advisor` implements
  `socialbenefits.advisor/Advisor` (defaults to `mock-advisor`).
  ...
  (-> (g/state-graph
      ...
      (g/add-node :commit
                   (fn [{:keys [request proposal]}]
                     (let [record {:applicant-id (:applicant-id proposal)
                                    :office-id (:office-id proposal)
                                    :op (:op proposal)
                                    :payload proposal}]
